...
impl Game {
    ...
    fn last_winner(&self) -> Board {
        let max_turn = self
            .boards
            .iter()
            .filter(|b| b.status.is_won())
            .map(|b| match b.status {
                BoardStatus::Won(_, t) => t,
                _ => 0,
            })
            .max()
            .unwrap();
        self.boards
            .iter()
            .find(|b| match b.status {
                BoardStatus::Won(_, t) => t == max_turn,
                _ => false,
            })
            .unwrap()
            .clone()
...
22 11 13  6  5
 2  0 12  3  7";
    ...
    fn test_game() {
        ...
        assert_eq!(game.play(), GameResult::Done);
        let b = game.last_winner();
        println!("{}", b);
        assert_eq!(b.status, BoardStatus::Won(1924, 14))
    }
}
